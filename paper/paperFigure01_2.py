...
# 1. Define an orange→white→dark-blue colormap
...
norm = mcolors.TwoSlopeNorm(vmin=-plotMat.max(), vcenter=0.0, vmax=plotMat.max()) # vmin 01
#  norm = mcolors.TwoSlopeNorm(vmin=plotMat.min(), vcenter=0.0, vmax=plotMat.max()) # vmin 02
...
plt.tight_layout()
figFile = plotFolder + 'label_track_corr_zeroCentered_o.pdf'
plt.tight_layout()
plt.savefig(figFile)
plt.close('all')
plt.show()
