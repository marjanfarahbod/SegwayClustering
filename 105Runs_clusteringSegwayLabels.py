...
totals = np.zeros([105,1])

for index in range(105):
    cluster_list = ann_info_list[index]['segway_anns']['clusters']
    for cluster in cluster_list:
        totals[index] += ann_info_list[index]['segway_anns']['clusters'][cluster].bp_count
...
for clustering in clusterings_list:
    ...
    label_ratios = np.zeros([len(label_list),1])
    for i, label in enumerate(label_list):
        ids = label.split('___')
        annID = int(ids[0])
        labelID = ids[1]
        
        label_ratios[i] = ann_info_list[annID]['segway_anns']['clusters'][labelID].bp_count/totals[annID]

    df = pd.DataFrame(label_ratios, index = label_list)
    ...
    figFile = plotFolder + clustering_name + '__genomeRatios.pdf'
    print(figFile)
    plt.savefig(figFile)
    plt.close('all')
...
########################################
# 10. To get some numbers for the report
########################################

# load the ward clustering

# get the count of each label

print(dendro_indices.keys())

indices = dendro_indices['ward_16']
...
for label in label_list:
    if 'Promoter' in label:
        pcount +=1
    if 'Enhancer' in label:
        ecount +=1
    if 'Quiescent' in label:
        qcount +=1
    if 'Reg' in label:
        rcount +=1
    if 'Low' in label:
...
'''
(pft): pick for training
promoter: ptf:[14___4,80___12]
enhancer: pft:[102___13, 67___0], [69___14, 12___10]pft, [12___10,69___14]
likely enhancer: [18___12]
bivalent: pft:[99___12, 26___10]
transcribed: pft:[63___7,56___5], [98___6, 60___4], [14___7,38___3]
week transcribed (?): [98___6, 60___4], [38___3,37___7]
cons_het: pft:[51___12,19___12], [68___9,71___5]
quiescent: pft:[98___0,12___7], [56___1,64___4]


(inv) promoterish: [12___12,41___4]
(inv) promoter_flanking: [31___11, 39___11]
(inv) broad weak k4me1: [27___6, 61___1]
(inv) transcribed? [70___11,98___2]

>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# from bottom to top
[82___15,5___14]: fachet
[81___4, 39___7]: inv - regper? there is hight CTCF


[6___12,31___11]: inv
[80___12,14___4]: promoter
'''
