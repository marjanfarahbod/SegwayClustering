...
trainCommand = 'segway train --include-coords=data/encodePilotRegions.hg19.bed --num-labels=15 --resolution=100 --minibatch-fraction=0.1 --num-instances=10 --prior-strength=%.1f --segtransition-weight-scale=%.3f --track-weight=%.4f --ruler-scale=100 --max-train-rounds=25 %s %s' %(ps, stws, tw, genomedataFile, outputTrainFolder)
