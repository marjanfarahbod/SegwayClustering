# updated version of annotation_comparison_scaled_01.py
#
# ITEMS IN THE CODE:
# ########################################
# 0. Initials
# 1. Unzip and sort the chmm files for each sample
# 2. do the comparison, save the matrix and plot
# 3. get the plots - with the updated classifier labels
...
for accession in accessionList:
    ...
    if ('testBatch105' in annotationFolder):
        continue
    else:
        
        chmmFile = annotation['chromFile']
        print(chmmFile)
...
book =  'EnhA1 EnhA2 EnhBiv EnhG1 EnhG2 EnhWk Het Quies ReprPC ReprPCWk TssA TssBiv TssFlnk TssFlnkD TssFlnkU Tx TxWk ZNF/Rpts'
chrom_class_list = book.split()

count = 0
for accession in accessionList[206:]: # for each sample
    print(count)
    count+=1
    ...
    if 'Batch105' in annotation['folder']:
        continue
    ...
    segway_cluster_list = list(label_term_mapping.keys())
    ...
    for label in segwayLabels:
        ...
        for cluster in segway_cluster_list:
            term = label_term_mapping[cluster]
            if term == label:
                finalTerm = '%s_%s' %(cluster, term)
                sortedClusterList.append(finalTerm)

    segway_cluster_list = sortedClusterList

    # get the segway annotation file
    segwayFile = annotation['bedFile'] + '.gz'
    os.system('gunzip %s' %(segwayFile))
    segwayFile = segwayFile[0:-3]
    ...
    if 'Batch105' in annotationFolder:
        ...
        chromFileTemp = annotation['chromFile']
        fileName = chromFileTemp.split('/')[-1]

        chromFile = annotationFolder + 'sorted_%s' %(fileName)
    ...
    segwayAccession = accession
    chromAccession = re.search('ENCFF.*\.', chromFile)[0][:-1]

    print(segwayAccession)
    print(chromAccession)

    segway_cluster_count = len(segway_cluster_list)
    chrom_class_count = len(chrom_class_list)
    ...
    chromLineInd = 1
    chromLine = linecache.getline(chromFile, chromLineInd)
    cfields = chromLine.split()
    cchr = cfields[0]
    ...
    while cchr != 'chr22' and schr != 'chr22': # while not at the end
        ...
        if cchr == schr: # if we are in the same chromosome
            ...
            if overlap > 0: # if there is an overlap
                ...
                cindex = chrom_class_list.index(cfields[3])
                ...
                while cstart > send and schr == cchr: # if at the beginning of chromosome, chrom is missing bps - I know segway won't be missing basepairs : if overlap < 0
                    #print('here')
                    segwayLineInd += 1
                    segLine = linecache.getline(segwayFile, segwayLineInd)
                    sfields = segLine.split()
    ...
    overlap_df = pd.DataFrame(overlap_mat, index = segway_cluster_list, columns = chrom_class_list)

    os.system('gzip %s' %(chromFile))
    os.system('gzip %s' %(segwayFile))

    outputFileName = 'overlap_segway_%s_chmm_%s_v04.pkl' %(segwayAccession, chromAccession)
    outputFile = annotationFolder + outputFileName
