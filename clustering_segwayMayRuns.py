...
runID = '5857d68b-e559-4776-9c12-a6e10aea7f76'
removeInd = runIDs.index(runID)
runIDs.remove(runID)
# The format of signal distribution is completely off
...
figFile = '%sclustergram_ward_May92runs.pdf' %(plotFolder)
print(figFile)
plt.savefig(figFile)
plt.close('all')
...
sns.set(font_scale = .4)
figFile = '%sclustergram_ward_May92runs_thr_zscore_classifierLabels_testrun.pdf' %(plotFolder)
print(figFile)
plt.savefig(figFile)
plt.close('all')
